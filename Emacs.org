...
* Chemacs2 Startup policy
I use the [[https://github.com/plexus/chemacs2][chemacs2]] emacs config switcher.
I like to control the .emacs.d/init.el which is written from this file by org-bable-tangle.
...
** The intent
I wish to run various .emacs configuration some just to test features.
I sometimes wish to run emacs in a specific msys2 enviroment.
Various of my subsystem need to be initialized with specific paths.
The remainder of Emacs.org and init.el should be written with standard configuration values set in the  user-custom-startup files.
The protocol is to name the file COMPUTER_SUBCASE-user-statup.el where the name COMPUTER_SUBCASE is the .emacs-profiles.el key.

The main git repo for this protocol will have various COMPUTER_SUBCASE-user-startup.el files to cover all use cases.
...
#+end_src
* Default Pathnames with Enviroment
Overrides
