...
* To Be Done :noexport:
  List of things to be done to this file
  - [ ] Review and cleanup Documenation
  - [ ] Revert unchanged buffers and direds
* REVISIONS
  Add Revert buffer section.
  IDO has been reinstalled
  Removing  IDO AC- code
  Removed IDO and AC- code by :tangle no
  Document core emacs configuration tools.
  MSYS64 lisp launch has been removed.
  Use slime implementation nged (commands) :env (environment vars)) with mingw start.
