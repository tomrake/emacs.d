...
*** Revision Narative
    - I am revising file generation from settings.org
    - settings.org is to be renamed Emacs.org
    - on saving this file will tangle code to init.el
    - THIS IS A Test of init.el generation.
...
*** Standard coding setup
    - In general the ~/.emacs.d and ~/quicklisp are symlinks from ~/Documents/Code
    - The source of Emacs startup code is largely controlled by ~/.emacs.d/setting.org
...
#+BEGIN_SRC emacs-lisp
  ...
  (package-initialize)
#+END_SRC
* use-package has specific loading requirements
